...
function DSCRLoanSection({ data, onShowDerivation }) {
  if (!data) return null;
  const { tierResults, criteriaRows, grossDSCR, ltv, pitia } = data;

  const tierColorMap = { conservative: "#00ccff", standard: "#00ff88", flexible: "#ffcc00", noratio: "#ff8844" };
  const overallQualifies = tierResults.some(t => t.qualifies);
  const bestTier = tierResults.find(t => t.qualifies);
    ...
    <div style={{ marginBottom: 28 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
        ...
        <span style={{
          ...
          background: overallQualifies ? "rgba(0,255,136,0.12)" : "rgba(255,68,68,0.12)",
          color: overallQualifies ? "#00ff88" : "#ff4444",
          border: `1px solid ${overallQualifies ? "rgba(0,255,136,0.25)" : "rgba(255,68,68,0.25)"}`,
        }}>
          {overallQualifies ? `✓ QUALIFIES — ${bestTier?.label} Tier` : "✗ NO QUALIFICATION"}
        </span>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 18 }}>
        {[
          { label: "Lender DSCR (Rent ÷ PITIA)", value: grossDSCR.toFixed(2) + "x", color: grossDSCR >= 1.25 ? "#00ff88" : grossDSCR >= 1.0 ? "#ffcc00" : "#ff4444" },
          { label: "Investor DSCR (NOI ÷ P&I)", value: data.investDSCR.toFixed(2) + "x", color: "rgba(255,255,255,0.6)" },
          { label: "LTV", value: ltv.toFixed(1) + "%", color: ltv <= 75 ? "#00ff88" : ltv <= 80 ? "#ffcc00" : "#ff4444" },
          { label: "PITIA / mo", value: fmt(pitia), color: "rgba(255,255,255,0.6)" },
          { label: "Rent-to-Value", value: data.rtv.toFixed(3) + "%", color: data.rtv >= 0.75 ? "#00ff88" : "#ff8844" },
        ].map(item => (
          <div key={item.label} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "10px 16px", flex: "1 1 140px", minWidth: 120 }}>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", fontFamily: "'Space Mono',monospace", letterSpacing: "0.08em", marginBottom: 4 }}>{item.label}</div>
            <div style={{ fontSize: 18, fontWeight: 700, fontFamily: "'Space Mono',monospace", color: item.color }}>{item.value}</div>
          </div>
        ))}
      </div>

      {/* Tier qualification grid */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 10, marginBottom: 18 }}>
        {tierResults.map(tier => (
          <div key={tier.key} style={{
            background: tier.qualifies ? `${tier.color}0d` : "rgba(255,255,255,0.02)",
            border: `1px solid ${tier.qualifies ? `${tier.color}30` : "rgba(255,255,255,0.07)"}`,
            borderRadius: 12, padding: "14px 16px",
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: tier.qualifies ? tier.color : "rgba(255,255,255,0.5)" }}>{tier.label}</div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginTop: 2 }}>{tier.sublabel}</div>
              </div>
              <div style={{
                fontSize: 10, fontWeight: 700, fontFamily: "'Space Mono',monospace",
                padding: "3px 9px", borderRadius: 20,
                background: tier.qualifies ? `${tier.color}20` : "rgba(255,68,68,0.1)",
                color: tier.qualifies ? tier.color : "#ff6666",
                border: `1px solid ${tier.qualifies ? `${tier.color}40` : "rgba(255,68,68,0.2)"}`,
                whiteSpace: "nowrap",
              }}>{tier.qualifies ? "✓ QUALIFIES" : "✗ FAILS"}</div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", fontFamily: "'Space Mono',monospace", marginBottom: 3 }}>
                {tier.passCount}/{tier.totalCount} criteria pass
              </div>
              {tier.criteria.map(c => (
                <div key={c.key} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 11 }}>
                  <span style={{ color: c.pass ? "#00ff88" : "#ff4444", flexShrink: 0 }}>{c.pass ? "✓" : "✗"}</span>
                  <span style={{ color: c.pass ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.4)", flex: 1 }}>{c.label}</span>
                  <span style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, color: c.pass ? "#00ff88" : "#ff6666", flexShrink: 0 }}>
                    {c.key.startsWith("ltv") ? ltv.toFixed(1)+"%" : c.key.startsWith("grossDSCR") ? grossDSCR.toFixed(2)+"x" : c.key === "rtv" ? data.rtv.toFixed(3)+"%" : ""}
                  </span>
                </div>
              ))}
            </div>
      ...
      {/* All criteria rows — clickable */}
      <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, overflow: "hidden" }}>
        <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", fontFamily: "'Space Mono',monospace", letterSpacing: "0.1em" }}>ALL CRITERIA — click any row to see derivation</span>
          <span style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", fontFamily: "'Space Mono',monospace" }}>Lender DSCR method: Rent ÷ PITIA</span>
        </div>
        {criteriaRows.map((c, i) => (
          <div key={c.key}
            onClick={() => c.derivation && onShowDerivation(c.derivation)}
            style={{
              display: "flex", justifyContent: "space-between", alignItems: "center",
              padding: "11px 16px", gap: 12,
              background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)",
              borderBottom: i < criteriaRows.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none",
              cursor: "pointer", transition: "background 0.12s",
            }}
            onMouseEnter={e => e.currentTarget.style.background = c.pass ? "rgba(0,255,136,0.04)" : "rgba(255,68,68,0.03)"}
            onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)"}
            title="Click to see derivation"
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
              <div style={{ width: 20, height: 20, borderRadius: "50%", background: c.pass ? "rgba(0,255,136,0.15)" : "rgba(255,68,68,0.15)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <span style={{ fontSize: 10, color: c.pass ? "#00ff88" : "#ff4444" }}>{c.pass ? "✓" : "✗"}</span>
              </div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: c.pass ? "rgba(255,255,255,0.85)" : "rgba(255,255,255,0.5)" }}>{c.label} <span style={{ fontSize: 9, color: "rgba(255,255,255,0.2)" }}>ⓘ</span></div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginTop: 1 }}>{c.sublabel}</div>
              </div>
            </div>
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 700, fontFamily: "'Space Mono',monospace", color: c.pass ? "#00ff88" : "#ff6666" }}>{c.value}</div>
              <div style={{ fontSize: 10, color: c.pass ? "#00ff88" : "#ff4444", marginTop: 1 }}>{c.pass ? "PASS" : "FAIL"}</div>
            </div>
          </div>
        ))}
      </div>

      <div style={{ marginTop: 10, fontSize: 11, color: "rgba(255,255,255,0.22)", lineHeight: 1.6 }}>
        ⓘ DSCR loans are investor loans that qualify based on property rental income, not personal income. Typical rates run 1–2% above conventional rates. Credit score, reserves (6–12 months PITIA), and entity structure (LLC vs personal) also affect eligibility. This is a screening tool — consult a DSCR-specialist lender for final qualification.
      </div>
    </div>
  );
}
